        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->integerNode('connect_timeout')
                    ->info('Seconds allowed for establishing the connection.')
                    ->defaultValue(10)
                ->end()
                ->integerNode('max_retries')
                    ->defaultValue(3)
                ->end()
                ->floatNode('retry_delay')
                    ->defaultValue(1.0)
                ->end()
                ->floatNode('max_retry_delay')
                    ->info('Ceiling in seconds for a single backoff, including a server-supplied Retry-After.')
                    ->defaultValue(30.0)
                ->end()
                ->floatNode('max_requests_per_second')
                    ->info('Client-side pacing of outgoing requests. 0 disables throttling.')
                    ->defaultValue(0.0)
                ->end()
